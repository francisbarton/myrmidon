...
#'  generally not just `NA`s, specifying custom join_by columns and so on).
#' But for now I just want to make a solid function that works and does this
#'  one job well, without lots of risks or much scope for data loss disasters.
#'
...
#'  refer to the column to be patched and the column with the replacement
#'  values. The column should be a vector column in both cases, and of the same
#'  `type` otherwise coercion to the lower level type may occur.
#'
...
patch_join <- function(dtf_x, dtf_y, patch_col) {
  ...
  by_cols <- setdiff(intersect(names(dtf_x), names(dtf_y)), patch_col)
  ...
  assert_that(typeof(dtf_x[[patch_col]]) == typeof(dtf_y[[patch_col]]))
  ...
  out <- dtf_x |>
    ...
    dplyr::mutate(
      across({{ patch_col }}, \(x) dplyr::coalesce(x, .data[["src_col"]]))
    ) |>
